...
tables the capacity model can train on. Regenerated on every vote (cheap for the reduced pool).

Tables written to ``outputs/capacity_game/``:
  ...
  * ``cases.csv``       -- pool cases with their patient features (dimension table);
  * ``case_labels.csv`` -- MODEL-READY: per case a surgeon-normalised capacity score
    (continuous + 0-1 + z), an ordinal tier, ``n_compares``, joined features, and
    (owner-only, last column) the hidden ``capacity_true`` for validation.
...
def build_case_labels(votes, cases, pool_ids, ratings_df=None, weights=None) -> pd.DataFrame:
    ...
    df["capacity_score"] = df["bt_strength"].where(df["bt_strength"].notna(), df["rating"])
    ...
    df["tier_idx"] = -1
    if seen.any():
        s01, sz = _scale(df.loc[seen, "capacity_score"])
        df.loc[seen, "score_01"] = s01
        df.loc[seen, "score_z"] = sz
        ranks = df.loc[seen, "capacity_score"].rank(method="first")
        n = int(seen.sum())
        if n >= N_TIERS:
    ...
    keep = ["case_id", *feat_cols] + (["capacity_true"] if "capacity_true" in cases.columns else [])
    df = df.merge(cases[keep], on="case_id", how="left")

    lead = ["case_id", "capacity_score", "score_01", "score_z", "tier", "tier_idx",
            "rating", "bt_strength", "n_compares"]
    has_truth = "capacity_true" in df.columns
    tail = [c for c in df.columns if c not in lead and c != "capacity_true"]
    ordered = [*lead, *tail] + (["capacity_true"] if has_truth else [])  # truth LAST = owner-only
    df = df[ordered]
    return df.sort_values("capacity_score", ascending=False).reset_index(drop=True)
...
def build_cases_dim(cases, pool_ids) -> pd.DataFrame:
    """Pool cases with patient-feature columns (truth columns dropped)."""
    pool = [int(c) for c in pool_ids]
