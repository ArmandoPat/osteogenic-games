"""Osteogenic-demand comparison game -- Streamlit UI.

Run from the repo root:
    streamlit run demand_game/app.py

Two synthetic planned operations are shown side by side; the surgeon picks the one with **higher
osteogenic demand** (how much new bone the construct must form to fuse). The winner stays, the
loser is replaced by a coverage-aware challenger, and every click updates a shared Elo ladder that
is binned into demand buckets for model training.
"""
...
os.environ.setdefault("BONEGRAFT_TABLE_PREFIX", "demand_")
...
CASES_PATH = Path(os.environ.get("DEMAND_CASES_PATH")
                  or (ROOT / "outputs" / "synthetic" / "synthetic_cases.csv"))
# Collected data (votes, roster, case pool, model-ready exports). Point DEMAND_DATA_DIR at
# PERSISTENT storage when deployed (e.g. /home/data/demand_game on Azure App Service) so the
# votes and roster survive restarts and redeploys.
OUT_DIR = Path(os.environ.get("DEMAND_DATA_DIR")
               or (ROOT / "outputs" / "demand_game"))
...
def _logo_uri() -> str:
    ...
    demand_game/assets/, else "" (the chrome then falls back to a text wordmark)."""
...
def _admin_passcode() -> str | None:
    ...
    DEMAND_ADMIN_PASSCODE env var. Returns None when nothing is configured, which
    ...
    try:
        ...
        pc = None
    return pc or os.environ.get("DEMAND_ADMIN_PASSCODE") or None
...
st.set_page_config(
    page_title="Medtronic · Osteogenic Demand Study",
    page_icon="🦴",
    layout="wide",
...
:root{
  /* DEMAND theme = purple & white (distinct from the capacity indigo/cyan/magenta). */
  --mdt-navy:#3B0A6B; --mdt-royal:#6D28D9; --mdt-blue:#7C3AED; --mdt-blue-600:#6D28D9;
  --mdt-blue-700:#5B21B6; --mdt-cyan:#9333EA; --mdt-magenta:#C084FC; --mdt-bright:#9333EA;
  --mdt-sky:#F3EAFE; --ink:#16233A; --muted:#5B6577;
  --line:#ECE3F7; --bg:#F7F3FD; --card:#FFFFFF;
...
.app-header{ display:flex; align-items:center; justify-content:space-between;
  background:
    radial-gradient(120% 160% at 88% -30%, rgba(192,132,252,.52), transparent 45%),
    radial-gradient(120% 150% at 60% 140%, rgba(147,51,234,.52), transparent 52%),
    linear-gradient(105deg,#3B0A6B 0%,#6D28D9 55%,#7C3AED 100%);
  color:#fff; padding:15px 22px; border-radius:16px; margin-bottom:16px;
  box-shadow:0 10px 26px rgba(59,10,107,.30); }
...
.hero{ background:
    radial-gradient(110% 140% at 92% -15%, rgba(192,132,252,.55), transparent 46%),
    radial-gradient(100% 130% at 12% 125%, rgba(147,51,234,.50), transparent 52%),
    linear-gradient(120deg,#34075F,#6D28D9 55%,#7C3AED);
  border-radius:22px; padding:40px 44px; color:#fff; box-shadow:0 16px 46px rgba(52,7,95,.32); }
...
.stButton > button[kind="primary"], button[data-testid="stBaseButton-primary"]{
  ...
  min-height:54px; font-size:1.02rem; box-shadow:0 6px 18px rgba(109,40,217,.32); }
.stButton > button[kind="primary"]:hover, button[data-testid="stBaseButton-primary"]:hover{
  transform:translateY(-1px); box-shadow:0 9px 24px rgba(109,40,217,.42); filter:saturate(1.06) brightness(1.03); }
.stButton > button[kind="secondary"], button[data-testid="stBaseButton-secondary"]{
  background:#fff; color:var(--mdt-blue-600); border:1.5px solid #E7DBF7; }
.stButton > button[kind="secondary"]:hover, button[data-testid="stBaseButton-secondary"]:hover{
  border-color:var(--mdt-blue); color:var(--mdt-blue); background:#F7F1FE; }
/* Higher-demand vote buttons: WHITE with a Medtronic violet->lavender gradient border */
.st-key-pick_a button, .st-key-pick_b button{
  ...
  box-shadow:0 5px 16px rgba(109,40,217,.16) !important; }
.st-key-pick_a button:hover, .st-key-pick_b button:hover{
  transform:translateY(-1px);
  background:linear-gradient(#F7F1FE,#F7F1FE) padding-box,
...
.pcard:hover{ box-shadow:0 12px 32px rgba(124,58,237,.16); border-color:#E4D6F6; transform:translateY(-2px); }
.pcard-slot{ position:absolute; top:13px; right:15px; width:26px; height:26px; border-radius:8px;
  ...
  box-shadow:0 3px 9px rgba(109,40,217,.30); }
...
.pcard-avatar{ width:50px; height:50px; border-radius:13px; flex:none;
  ...
  border:none; box-shadow:0 4px 12px rgba(109,40,217,.24); }
...
.badge{ display:inline-block; padding:4px 10px; border-radius:20px; font-size:12px; font-weight:500;
  background:#F3EEFB; color:#39465A; border:1px solid #E7DEF6; margin:0 5px 5px 0; }
...
.vs-badge{ width:46px; height:46px; border-radius:50%; color:var(--mdt-royal); font-weight:800;
  ...
  box-shadow:0 5px 16px rgba(109,40,217,.22); }
...
.side-id{ display:flex; align-items:center; gap:8px; font-weight:700; color:var(--ink);
  background:var(--mdt-sky); border:1px solid #E4D6F6; border-radius:10px; padding:8px 12px; font-size:14px; }
...
<div class="app-header">
  <div class="brand">{brand}
    ...
    <span class="app-title">Osteogenic Demand Study</span></div>
...
<div class="hero">
  {hero_logo}
  <h1>Which operation needs more new bone?</h1>
  <p>You'll see two planned operations side by side. Pick the one with the higher
     <b>osteogenic demand</b> &mdash; how much new bone the construct must form to fuse. Each choice
  ...
  <div class="hero-steps">
    <div class="hero-step"><div class="n">STEP 01</div><div class="t">Compare</div>
      <div class="d">Two planned operations, side by side.</div></div>
    <div class="hero-step"><div class="n">STEP 02</div><div class="t">Choose</div>
      <div class="d">Click the higher-demand case &mdash; or tap &larr; / &rarr;.</div></div>
...
with st.sidebar:
    st.markdown(f"<div class='side-brand'>{_brand_lockup(dark=False, cls='side-logo')}"
                f"<span>Demand Study</span></div>", unsafe_allow_html=True)
    ...
    if is_admin:  # owner-only controls; hidden from surgeons
        ...
        n_buckets = st.selectbox("Demand buckets", [3, 5, 10], index=1)
        smart = st.toggle(
...
# --------------------------------------------------------------------------------- COMPARE -
with compare_tab:
    st.markdown("<div class='sec-title'>Which case places the higher osteogenic demand?</div>",
                unsafe_allow_html=True)
    st.caption("Osteogenic demand = how much new bone the construct must form to fuse. "
               "Judge the surgery; ignore the patient's biology.")
...
with insights_tab:
    ...
    if len(votes) == 0:
        ...
        st.markdown("<div class='sec-title'>Demand buckets</div>", unsafe_allow_html=True)
        dist = (ranked.groupby("bucket").size().reindex(labels, fill_value=0)
                .rename_axis("bucket").reset_index(name="cases"))
        chart = (
            alt.Chart(dist)
            .mark_bar(color="#7C3AED", cornerRadiusTopLeft=5, cornerRadiusTopRight=5)
            .encode(
        ...
        with lb1:
            st.markdown("<div class='sec-title'>Highest demand</div>", unsafe_allow_html=True)
            st.dataframe(ranked.nlargest(12, "rating")[show_cols],
                         hide_index=True, width="stretch")
        with lb2:
            st.markdown("<div class='sec-title'>Lowest demand</div>", unsafe_allow_html=True)
        ...
        with col_b:
            st.markdown("<div class='sec-title'>Model-ready exports</div>",
                        unsafe_allow_html=True)
            try:  # data dir may live outside the repo (persistent storage on deploy)
                out_disp = OUT_DIR.relative_to(ROOT)
            except ValueError:
                out_disp = OUT_DIR
            st.caption(f"Auto-written to `{out_disp}` on every vote — tidy, "
                       "surgeon-normalised tables ready for the demand model.")
